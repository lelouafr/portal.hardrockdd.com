...
namespace portal.Models.Views.JC.Job.Revenue
{
    public class JobRevenueListViewModel
    {

        public JobRevenueListViewModel()
        {

        }

        public JobRevenueListViewModel(DB.Infrastructure.ViewPointDB.Data.Job job)
        {
            if (job == null) return;

            List = job.Contract.InvoiceDetails.Where(f => f.JCTransType != "OC").Select(s => new JobRevenueViewModel(s)).ToList();
        }

        public JobRevenueListViewModel(DB.Infrastructure.ViewPointDB.Data.Job job, DateTime mth)
        {
            if (job == null) return;

            List = job.Contract.InvoiceDetails.Where(f => f.JCTransType != "OC" && f.Mth == mth).Select(s => new JobRevenueViewModel(s)).ToList();
        }

        public List<JobRevenueViewModel> List { get; set; }
    }

    public class JobRevenueViewModel
    {
        public JobRevenueViewModel()
        {

        }

        public JobRevenueViewModel(DB.Infrastructure.ViewPointDB.Data.JCInvoiceDetail item, bool UnPosted = false)
        {
            if (item == null)
                return;

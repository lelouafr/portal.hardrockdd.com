...
namespace portal.Models.Views.DailyTicket
{
    public class DailyTicketListSummaryViewModel
    {
        ...
        public DailyTicketListSummaryViewModel(CompanyDivision division, List<DB.DailyTicketStatusEnum> status, DB.TimeSelectionEnum timeSelection)
		{
			if (division == null) throw new System.ArgumentNullException(nameof(division));
			if (status == null) throw new System.ArgumentNullException(nameof(status));
			...
			var dtCompanies = new List<byte>();
            var dtDivisions = new List<byte>(); 
            division.SubDivisions.ForEach(d => {  
                if (!dtCompanies.Any(f => f == d.DTCo))
                    dtCompanies.Add(d.DTCo); 
            });

			var statusList = status.Select(s => (int)s).ToList();

			var vtickets = db.vDailyTickets.Where(f => dtCompanies.Contains(f.DTCo) && f.WorkDate >= asofDate && statusList.Contains((int)f.Status)).OrderByDescending(o => o.WorkDate).ToList();
			var list = vtickets.Select(s => new DailyTicketSummaryViewModel(s)).ToList();
			Tickets.AddRange(list);
			NewTicket = new DailyTicketCreateViewModel();
			TimeSelection = timeSelection;
        ...
        public List<DailyTicketSummaryViewModel> Tickets { get; } = new List<DailyTicketSummaryViewModel>();

        public DailyTicketCreateViewModel NewTicket { get; set; } = new DailyTicketCreateViewModel();

        [UIHint("EnumBox")]
        [Display(Name = "Time Range")]
        public DB.TimeSelectionEnum TimeSelection { get; set; } = DB.TimeSelectionEnum.LastMonth;

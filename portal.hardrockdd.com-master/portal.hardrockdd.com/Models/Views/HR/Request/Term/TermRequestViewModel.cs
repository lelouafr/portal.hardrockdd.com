...
namespace portal.Models.Views.HR.Request.Term
{
    public class TermRequestListViewModel
    {
        ...
        public TermRequestListViewModel(List<Code.Data.VP.HRTermRequest> list)
        {

            List = list.Select(s => new TermRequestViewModel(s)).ToList();

        }
        public TermRequestListViewModel(Code.Data.VP.WebUser user)
        {
        ...
        public TermRequestListViewModel(Code.Data.VP.HRResource supervisor)
        {
            List = supervisor
                .DirectReports
                .SelectMany(s => s.TermRequests.Where(f => f.Status != HRTermRequestStatusEnum.Completed))
                .Select(s => new TermRequestViewModel(s)).ToList();
        }

        public TermRequestListViewModel(Code.Data.VP.VPEntities db)
        {
            var termList = db.HRTermRequests.Where(f => f.tStatusId != (int)HRTermRequestStatusEnum.Completed).ToList();
            List = termList.Select(s => new TermRequestViewModel(s)).ToList();

        }


        public List<TermRequestViewModel> List { get; set; }
    }
    ...
    public class TermRequestViewModel
    {
        ...
        public TermRequestViewModel(Code.Data.VP.HRTermRequest request)
        {
            if (request == null)
                return;
        ...
        public TermRequestViewModel ProcessUpdate(Code.Data.VP.VPEntities db, System.Web.Mvc.ModelStateDictionary modelState)
        {

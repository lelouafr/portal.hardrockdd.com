...
namespace portal
{
    ...
    public static partial class StaticFunctions
    {
        ...
        public static void SetCurrentDivision(int divisionId)
        {
            ...
            using var db = new VPContext();
            var payEmp = GetCurrentEmployee();
            var cacheCompany = db.CompanyDivisions.First(f => f.DivisionId == divisionId);

            SetCurrentCompany(cacheCompany.HQCo);
        ...
        public static CompanyDivision GetCurrentDivision()
        {
			...
			if (MemoryCache.Default[memKey] is DB.Infrastructure.ViewPointDB.Data.CompanyDivision cacheObj)
            {
                //if (cacheObj.DivisionId == GetCurrentEmployee().DivisionId)
					return cacheObj;
			}

            using var db = new VPContext();
            var payEmp = GetCurrentEmployee();
            cacheObj = db.CompanyDivisions.FirstOrDefault(f => f.DivisionId == (payEmp.DivisionId));
			cacheObj ??= db.CompanyDivisions.FirstOrDefault(f => f.DivisionId == 1);
            ...
            systemCache.Set(memKey, cacheObj, policy);
            return cacheObj;
        }

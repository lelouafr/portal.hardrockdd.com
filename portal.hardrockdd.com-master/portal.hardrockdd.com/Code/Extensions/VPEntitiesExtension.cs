...
namespace portal.Code.Data.VP
{
    ...
    {
        public VPEntities(ObjectContext objectContext, bool dbContextOwnsObjectContext)
                ...
                foreach (var entry in entries)
                {
                    ...
                    foreach (var key in keyNames)
                    {
                        ...
                        if (prop == typeof(DateTime))
                            keyString += string.Format(AppCultureInfo.CInfo(), "[{0}] = \"{1:MM/dd/yy}\"", key.Name, dbEntity.GetValue<object>(key.Name));
                        else
                            keyString += string.Format(AppCultureInfo.CInfo(), "[{0}] = \"{1}\"", key.Name, dbEntity.GetValue<object>(key.Name));
                    }
                    if (entry.Entity.GetType().GetProperties().Any(f => f.Name == "KeyID"))
                    {
                        ...
                        keyString += string.Format(AppCultureInfo.CInfo(), "[{0}] = \"{1}\"", "KeyID", entry.CurrentValues.GetValue<object>("KeyID"));
                    }
                    keyString += " />";
                    if (keyString == null)
                    {
        ...
        public int GetNextId(string tableName, string fieldName, string subKey = "")
        {
            if (IdList == null)
            {
                IdList = GetIdList();
            }
            var key = string.Format("{0}{1}_{2}", subKey, tableName, fieldName);
            if (IdList.ContainsKey(key))
            {
                var val = IdList[key];
                val++;
                SetNextId(tableName, fieldName, val, subKey);
                return val;
            }
            else
            {
                SetNextId(tableName, fieldName, -1, subKey);
            }
            return -1;
        }

        public void SetNextId(string tableName, string fieldName, int value = -1, string subKey = "")
        {
            var memKey = "GetIdList";
            ...
            systemCache.Set(memKey, IdList, policy);

        }

        private Dictionary<string, int> GetIdList()
        {
            var memKey = "GetIdList";

            if (!(MemoryCache.Default[memKey] is Dictionary<string, int> cachedList))
            {
                cachedList = new Dictionary<string, int>();
                CacheItemPolicy policy = new CacheItemPolicy
                {
                    AbsoluteExpiration = DateTime.Now + TimeSpan.FromDays(1)
                };
                systemCache.Set(memKey, cachedList, policy);
                return cachedList;
            }
        ...
        { 
            get
            {
                if (_CurrentUserId == null)
                    _CurrentUserId = StaticFunctions.GetUserId();
        ...
        public Employee GetCurrentEmployee()
        {
            ...
            if (_employee.DivisionId == null)
            {
                _employee.DivisionId = 1;
                var division = this.CompanyDivisions.FirstOrDefault(f => f.DivisionId == 1);
                _employee.Division = division;
                this.BulkSaveChanges();
            }
        ...
        public HQCompanyParm? GetCurrentCompany()
        {
            var company = new HQCompanyParm();
            if (!string.IsNullOrEmpty(CurrentUserId))
            {
                ...
                if (resource != null)
                {
                    ...
                    company = employee?.Division?.HQCompany;
                    var tmpComp = StaticFunctions.GetCurrentCompany();
                    if (company.HQCo != StaticFunctions.GetCurrentCompany().HQCo)
                    {
                        company = this.HQCompanyParms.FirstOrDefault(f => f.HQCo == tmpComp.HQCo);
                    }
                }
                if (company == null)
                    company = this.HQCompanyParms.FirstOrDefault();
            }

            return company;
        }

        public Code.Data.VP.WebUser? GetCurrenUser()
        {
        ...
        public WebApplicant AddApplicant()
        {
            ...
            app.ApplicantId = this.GetNextId(WebApplicant.BaseTableName, 1);

            this.WebApplicants.Add(app);
            return app;
        }

        public WebApplicant AddApplicant(HRResource resource)
        {
            ...
            if (applicant.Applications.Any())
            {
                ...
                if (applicant.HRRefId < 900000)
                {
                    lastApp.Status = DB.WAApplicationStatusEnum.Hired;
                }
            }
            return applicant;
        }

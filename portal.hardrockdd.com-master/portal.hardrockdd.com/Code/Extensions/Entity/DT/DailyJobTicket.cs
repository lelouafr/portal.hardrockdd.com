...
namespace portal.Code.Data.VP
{
    public partial class DailyJobTicket
    {
        ...
        public void UpdateEmployeesReportTo()
        {
            ...
            var employeeList = DailyTicket.DailyJobEmployees.Select(s => s.PREmployee).Distinct().Where(f => f.EmployeeId != crewLeader.EmployeeId).ToList();
        ...
        public DTJobPhase AddJobPhase(string phaseId = "", int? taksId = null)
        {
            ...
            {
                var costTypes = new int[] { 1, 2, 3, 13, 31, 32, 34, 35 };
        ...
        public void CalculateRigUtilization()
        {
            ...
            var dtTasks = db.DTJobPhases.Where(f => f.tWorkDate == WorkDate &&
                                                    ...
                                                    (f.JobTicket.DailyTicket.tStatusId == (int)DB.DailyTicketStatusEnum.Submitted ||
                                                    f.JobTicket.DailyTicket.tStatusId == (int)DB.DailyTicketStatusEnum.Approved ||
                                                    f.JobTicket.DailyTicket.tStatusId == (int)DB.DailyTicketStatusEnum.Processed))

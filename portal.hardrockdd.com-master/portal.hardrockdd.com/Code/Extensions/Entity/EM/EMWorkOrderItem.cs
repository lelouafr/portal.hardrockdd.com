...
namespace portal.Code.Data.VP
{
    public partial class EMWorkOrderItem
    {
        ...
        public static EMWorkOrderItem Create(VPEntities db, SMRequestLine requestLine, bool addToExisting = true)
        {
            if (db == null)
                return null;
            if (requestLine == null)
            ...
            requestLine.Status = DB.SMRequestLineStatusEnum.WorkOrderCreated;
            return item;
        }
        ...
        public string StatusCodeId
        {
            ...
            set
            {
                ...
                {
                    var requestStatus = (DB.SMRequestLineStatusEnum)SMRequestLines.Min(min => min == null ? 0 : min.StatusId);
                    if (StatusCode.StatusType == "N")
                    {
                        if (StatusCode.Description.ToLower().Contains("pending"))
                            requestStatus = DB.SMRequestLineStatusEnum.WorkOrderCreated;

                        if (StatusCode.Description.ToLower().Contains("progress"))
                            requestStatus = DB.SMRequestLineStatusEnum.WorkOrderInProcess;
                    ...
                    if (StatusCode.StatusType == "F")
                    {
                        ...
                        if (StatusCode.Description.ToLower().Contains("complete"))
                            requestStatus = DB.SMRequestLineStatusEnum.WorkOrderCompleted;

                        if (StatusCode.Description.ToLower().Contains("canceled"))
                            requestStatus = DB.SMRequestLineStatusEnum.WorkOrderCanceled;

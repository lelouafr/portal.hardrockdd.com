...
namespace portal.Code.Data.VP
{
    public partial class EMWorkOrder
    {
        public static EMWorkOrder Init(SMRequestLine requestLine)
        {
            ...
            using var db = Code.Data.VP.VPEntities.GetDbContextFromEntity(requestLine);
            result.WorkOrderId = NextWorkOrderSeq(db, requestLine.Equipment);

            return result;
        }

        private static string NextWorkOrderSeq(VPEntities db, Equipment equipment)
        {
            var workOrderId = "";
        ...
        public static EMWorkOrder Create(VPEntities db, SMRequestLine requestLine)
        {
            if (db == null)
                return null;
            if (requestLine == null)

...
namespace portal.Areas.HumanResource.Models.PositionRequest
{
    public class ApplicationListViewModel
    {
        ...
        public ApplicationListViewModel(HRPositionRequest request)//HRPositionApplicant
        {
            if (request == null)
                return;

            var list = request.db.WebApplications.Where(f => f.tIsActive &&  f.AppliedPositions.Any(f => f.tPositionCodeId == request.PositionCodeId)).ToList();

            List = request.WAApplicants.Select(s => new ApplicationViewModel(s)).ToList();
            list = list.Where(f => f.Status == DB.WAApplicationStatusEnum.Approved && !List.Any(a => a.ApplicantId == f.ApplicantId && a.ApplicationId == f.ApplicationId)).ToList();
    ...
    public class ApplicationViewModel
    {
        ...
        public ApplicationViewModel(HRPositionApplicant applicant)
        {
            if (applicant == null)
                return;
        ...
        public ApplicationViewModel(HRPositionRequest request, WebApplication application)
        {
        ...
        [UIHint("EnumBox")]
        [Display(Name = "Status")]
        public DB.HRPositionApplicantStatusEnum Status { get; set; }
        ...
        internal ApplicationViewModel ProcessUpdate(VPContext db, System.Web.Mvc.ModelStateDictionary modelState)
        {

...
namespace portal.Areas.Project.Models.Job
{
    public class ProjectListViewModel
    {
        ...
        public ProjectListViewModel(List<vJob> jobs, VPContext db)
        {
            ...
            List = jobs.OrderBy(f => f.JobId).ToList().Select(s => new ProjectViewModel(s)).ToList();//s.Where(f => f.JCCo == company.HQCo)
        }

        public ProjectListViewModel(HQCompanyParm company, byte status, VPContext db)
        {
            if (company == null) throw new System.ArgumentNullException(nameof(company));
            if (db == null) throw new System.ArgumentNullException(nameof(db));

            JCCo = company.HQCo;
            List = db.vJobs.Where(f => f.JobStatus == status).OrderBy(f => f.JobId).ToList().Select(s => new ProjectViewModel(s)).ToList();// f.JCCo == company.HQCo && 
    ...
    public class ProjectViewModel
    {
        ...
        public ProjectViewModel(vJob job)
        {

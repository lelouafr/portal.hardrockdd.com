...
namespace portal.Areas.Project.Models.Job
{
    public class JobListViewModel
    {
        ...
        //public JobListViewModel(HQCompanyParm company, VPContext db)
        //{
        ...
        //    List = db.vJobs.OrderBy(f => f.JobId).ToList().Select(s => new JobViewModel(s)).ToList();//s.Where(f => f.JCCo == company.HQCo)
        //}

        public JobListViewModel(List<vJob> jobs, VPContext db)
        {
            if (jobs == null) throw new System.ArgumentNullException(nameof(jobs));
            if (db == null) throw new System.ArgumentNullException(nameof(db));

            List = jobs.OrderBy(f => f.JobId).ToList().Select(s => new JobViewModel(s)).ToList();//s.Where(f => f.JCCo == company.HQCo)
        }

        //public JobListViewModel(List<DB.Infrastructure.ViewPointDB.Data.Job> jobs)
        //{
        //    if (jobs == null) throw new System.ArgumentNullException(nameof(jobs));
        ...
        public JobListViewModel(HQCompanyParm company, byte status, VPContext db)
        {
            if (company == null) throw new System.ArgumentNullException(nameof(company));
            if (db == null) throw new System.ArgumentNullException(nameof(db));

            JCCo = company.HQCo;
            List = db.vJobs.Where(f => f.JobStatus == status).OrderBy(f => f.JobId).ToList().Select(s => new JobViewModel(s)).ToList();// f.JCCo == company.HQCo && 
    ...
    public class JobViewModel
    {
        ...
        public JobViewModel(DB.Infrastructure.ViewPointDB.Data.Job job)
        {
            ...
            RigId = job.RigId;
            HasAttachment = job.HQAttachment?.Files?.Any() ?? false;
            DivisionId = job.DivisionId;

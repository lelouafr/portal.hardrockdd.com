...
namespace portal.Areas.Job.Models.Cost
{
    public class ListViewModel
    {
        ...
        public ListViewModel(DB.Infrastructure.ViewPointDB.Data.Job job, bool inlcudeUnPosted = true)
        {
            if (job == null)
                return;
            
            List = job.ActualJobCosts().Select(s => new ViewModel(s)).ToList();

            JCCo = job.JCCo;
            JobId = job.JobId;

            var jobCosts = job.ActualJobCosts();
            List = jobCosts.Select(s => new ViewModel(s)).ToList();
            if (inlcudeUnPosted)
            {
                var jobUnPostCosts = job.UnPostedJobCosts();
                List.AddRange(jobUnPostCosts.Select(s => new ViewModel(s, true)).ToList());
            }

        }

        public ListViewModel(DB.Infrastructure.ViewPointDB.Data.Job job, DateTime mth, bool inlcudeUnPosted = true)
        {
    ...
    public class ViewModel
    {
        ...
        public ViewModel(DB.Infrastructure.ViewPointDB.Data.JobCost cost, bool UnPosted = false)
        {
            if (cost == null)
            {
                return;

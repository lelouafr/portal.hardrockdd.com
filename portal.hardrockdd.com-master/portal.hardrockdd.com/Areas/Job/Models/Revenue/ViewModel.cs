...
namespace portal.Areas.Job.Models.Job.Revenue
{
    public class ListViewModel
    {
        ...
        public ListViewModel(DB.Infrastructure.ViewPointDB.Data.Job job)
        {
            if (job == null) return;

            List = job.Contract.InvoiceDetails.Where(f => f.JCTransType != "OC" && f.BilledAmt != 0).Select(s => new ViewModel(s)).ToList();
        }

        public ListViewModel(DB.Infrastructure.ViewPointDB.Data.Job job, DateTime mth)
        {
            if (job == null) return;

            List = job.Contract.InvoiceDetails.Where(f => f.JCTransType != "OC" && f.BilledAmt != 0 && f.Mth == mth).Select(s => new ViewModel(s)).ToList();
        }
    ...
    public class ViewModel
    {
        ...
        public ViewModel(DB.Infrastructure.ViewPointDB.Data.JCInvoiceDetail item, bool UnPosted = false)
        {
            if (item == null)
                return;

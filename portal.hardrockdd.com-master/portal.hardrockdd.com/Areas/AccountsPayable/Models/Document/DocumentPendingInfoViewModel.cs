...
{
    public class DocumentRequestInfoViewModel
    {
        public DocumentRequestInfoViewModel()
        {

        }

        public DocumentRequestInfoViewModel(APDocument document)
        {
        ...
        internal DocumentRequestInfoViewModel ProcessUpdate(VPContext db, ModelStateDictionary modelState, ControllerContext controllerContext)
        {
            ...
            if (updObj != null)
            {
                ...
                {
                    db.BulkSaveChanges();
                    return new DocumentRequestInfoViewModel(updObj);
                }
                catch (Exception ex)
                {
        ...
        private void EmailRequestInfo(APDocument document, ControllerContext controllerContext)
        {
            ...
            try
            {
                ...
                {
                    Body = Code.EmailHelper.RenderViewToString(controllerContext, "Email/RequestedInfo", model, false),
                    IsBodyHtml = true,
                    Subject = string.Format(AppCultureInfo.CInfo(), "Requested Invoice Info {0}", model.Info.APReference),
                };

...
(function (factory) {
    ...
    $.extend(TreeGrid.prototype, {
        fnConstruct: function (oInit) {
            //console.log('fnConstruct TreeGrid');
            ...
            var expandLevel = $(this.s.expandLevel);
            ...
            var expandNodeToLevel = function (elem, toLayer) {
                var parentTr = getParentTr(elem);
                var td = $(dataTable.cell(getParentTd(elem)).node());
            ...
            var resetParentData = function (parentRow) {
                var index = parentRow.index();
                var childernData = [];
                var body = dataTable.table().body();
                $(body).find("[parent-index='" + index + "']").each(function () {

                    var row = $(this);
                    var dtRow = dataTable.row(row);
                    var isOpen = $(this).find('.treegrid-control-open').length !== 0;
                    if (isOpen) {

                        dtRow.data()["children"] = resetParentData(dtRow);
                    }
                    childernData.push(dtRow.data());
                });
                parentRow.data()["children"] = childernData;
            }
            ...
            var expandNode = function (elem, target) {
                if (!$(elem).html()) {
                    return;
                }
                ...
                var parentTr = getParentTr(target);
                var parentTrId = getTrId();
                $(parentTr).attr('tg-id', parentTrId);
                var row = dataTable.row(parentTr);
                var index = row.index();
                var data = row.data();
                data["treeGrid_expanded"] = true;
                var td = $(dataTable.cell(getParentTd(target)).node());
                ...
                if (data.children && data.children.length) {
                    ...
                    setTimeout(function () {
                        subRows.forEach(function (node) {
                            initInputPlugins(node);
                        });
                    }, 0);
                    var tableElem = $(elem).closest('.dataTables_wrapper');
                    //console.log('expanded', tableElem);
                    Table().setTableHeight($(tableElem));
                    //dataTable.order();
                    //dataTable.draw();
                }
            }

            var collapseNode = function (elem, target) {
                var selectedIndexes = [];
                select && (selectedIndexes = dataTable.rows({ selected: true }).indexes().toArray());

...
/* 00.5. Handle Check Sidebar
------------------------------------------------ */
var handleCheckSidebar = function () {
...
var generateSlimScroll = function(element) {
	...
	if (!dataHeight) {
		...
		dataHeight = windowHeight - distance - 20;
        if (dataHeight < 300) {
			dataHeight = 300;
        }
		//console.log(scrollTop, elementOffset, windowHeight, distance, dataHeight);
	}
	else {
...
var handleSidebarMinify = function () {
	$(document).on('click', '[data-click=sidebar-minify]', function(e) {
		...
		if ($(targetContainer).hasClass(sidebarClass)) {
			...
			$(targetContainer).addClass(sidebarClass);
            lscache.set('SideBar', 'page-sidebar-minified');
			if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
...
var handlePanelAction = function() {
	...
	$(document).on('click', '[data-click=panel-reload]', function (e) {
		...
		if (!$(target).hasClass('panel-loading')) {
			...
			var targetReloadUrl = $(target).attr('data-reloadurl');
				...
				setTimeout(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
							...
							$(target).find('[data-selectinit]').each(function () {
								$(this).data('flushcombo', 'true');
								$(this).attr('data-flushcombo', 'true');
							});
	...
	$(document).on('click', '[data-click=panel-expand]', function (e) {
		...
		if ($('body').hasClass('panel-expand') && $(target).hasClass('panel-expand')) {
			...
			$(target).find('img').each(function () { resizeImage(this) });
		} else {
			$('body').addClass('panel-expand');
			$(this).closest('.panel').addClass('panel-expand');
			var tbHeight = $(target).height();
			tbHeight -= 150;
			$(target).find('iframe').css("height", tbHeight + "px");
			$(target).find('img').each(function () { resizeImage(this) });
	...
	$(document).on('click', '.btnExpand', function (e) {
		...
		if ($('body').hasClass('card-expand') && $(target).hasClass('card-expand')) {
			$('#page-container').show();
			$(target).remove();
			
		}
		else {
			$(target).attr('data-sourcetarget', 'cardExpanded');
			var target = $(target).clone();
			$(target).removeAttr('data-sourcetarget');
			$('body').append($(target));
			$('#page-container').hide();
        }
		...
		if ($('body').hasClass('card-expand') && $(target).hasClass('card-expand')) {
			$('body, .card').removeClass('card-expand');
			$(target).removeAttr('style');
			$(targetBody).removeAttr('style');
			target = $('body').find('[data-sourcetarget=cardExpanded]')[0];

		} else {
			$('body').addClass('card-expand');
			$(target).addClass('card-expand');
			if ($(targetBody).length !== 0 && targetTop != 40) {
				var finalHeight = 40;
				$(target).find(' > *').each(function () {
					var targetClass = $(this).attr('class');

					if (targetClass != 'card-header' && targetClass != 'card-body') {
						finalHeight += $(this).height() + 30;
					}
				});
				if (finalHeight != 40) {
		...
		var tbHeight = $(target).height() - targetTop;
		$(target).find('iframe').css("height", tbHeight + "px");
		$(target).find('img').each(function () { resizeImage(this) });
	...
	// expand
	$(document).on('hover', '[data-click=panel-popout]', function (e) {
		if (!$(this).attr('data-init')) {
			$(this).tooltip({
				title: 'Expand / Compress',
				placement: 'bottom',
	...
	$(document).on('click', '[data-click=panel-popout]', function (e) {
		e.preventDefault();

		var localtarget = $(this).closest('.panel');
		var panel = $(localtarget).parent();

		var w = window.open("/Home/Blank");
		setTimeout(function () {
			var content = $(w.document).find("#content");
			content.html(panel.html());
			//$(w.document.body).find('[data-click=panel-expand]').click();
			panel.hide();
			w.document.querySelectorAll("[data-click='panel-expand']")[0].click();
			$(panel).find('[data-click=panel-collapse]')[0].click();			
		}, 100);
	});


	// history
	$(document).on('hover', '[data-click=panel-history]', function (e) {
		if (!$(this).attr('data-init')) {
			$(this).tooltip({
				title: 'Audit Log',
				placement: 'bottom',
	...
	$(document).on('click', '[data-click=panel-history]', function (e) {
		e.preventDefault();
		var target = $(this).closest('.panel');
		if (!$(target).hasClass('panel-loading')) {
			var model = $('#auditLog');
			...
			var data = {
				EntityName: $(this).data('auditentityname'),
				EntityKeyString: $(this).data('auditentitykeystring')
			};
					...
					if (storageData) {
						$.each(storageData, function(index, data) {
							...
							if ($(targetId).length !== 0) {
                                ...
                                $(targetId).detach().appendTo(targetColumn);
...
var handlePopUpButton = function () {
	$(document)
		.off('click', '.btnPopup')
		.on('click', '.btnPopup', function () {
			console.log('Popup clicked');
			var url = $(this).data('url');
			if (url !== undefined) {
				url += "?";
				$.each($(this).data(), function (field, value) {
					if (field !== 'url') {
						url += field + '=' + value + "&";
					}
				});
				var randomnumber = Math.floor((Math.random() * 100) + 1);
				window.open(url, '_blank', 'PopUp' + randomnumber +',toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=1200,height=650');
			}
		});
}
...
var App = function () {
	...
	return {
		...
		initForgeryToken: function (token) {
			console.log("initForgeryToken");
			//var isAbsoluteURI = new RegExp('^(?:[a-z]+:)?//', 'i');
			//$.ajaxSetup({
		...
		initControllerToken: function () {
			var isAbsoluteURI = new RegExp('^(?:[a-z]+:)?//', 'i');
			$.ajaxSetup({
				beforeSend: function (xhr) {
					if (!isAbsoluteURI.test(this.url)) {
					//	console.log("initControllerToken", this.url);
						var verificationToken = $("#content").find('[name="__RequestVerificationToken"]')[0];
						var controllerToken = $(document).find('#controllerTokenId')[0];

						//only add header to relative URLs
						xhr.setRequestHeader('ControllerTokenId', $(controllerToken).val());
						xhr.setRequestHeader('validateantiforgerytokenonallposts', $(verificationToken).val() );

    ...
    public partial class DTJobPhaseCost
    {
        ...
        public decimal? PayrollValue
        {
            ...
            set
            {
                ...
                {
                    if (DailyJobTask.JobTicket.DailyTicket.Status == DailyTicketStatusEnum.Approved ||
                        DailyJobTask.JobTicket.DailyTicket.Status == DailyTicketStatusEnum.Processed)
                    {
                        tValueAdj = value;
                        tValue ??= value;
                    }
        ...
        public void UpdateBudgetCode()
        {
            ...
            if (PhaseId != null && BudgetCodeId == null)
            {
                ...
                else if (JCCType == 34)
                {
                    ...
                    {
                        description = string.Format("{0} {1:F3}", DailyJobTask.JobPhase.Description, DailyJobTask.PassSize);
                        bgtCode = Job.JCCompanyParm.HQCompanyParm.PMCompanyParm.BudgetCodes.FirstOrDefault(f => f.Prefix == prefix && f.PhaseId == phaseId && f.CostTypeId == JCCType && f.Description == description);
                    }
                    if (bgtCode == null)
                    {
                        description = string.Format("{0}", DailyJobTask.JobPhase.Description);
                        bgtCode = Job.JCCompanyParm.HQCompanyParm.PMCompanyParm.BudgetCodes.FirstOrDefault(f => f.Prefix == prefix && f.PhaseId == phaseId && f.CostTypeId == JCCType && f.Description == description);
                    }
                }

...
namespace DB.Infrastructure.ViewPointDB.Data
{
    ...
    {
        public static string BaseTableName { get { return "budCMTH"; } }
        ...
        public CMTranStatusEnum Status
        {
            get
            {
                return (CMTranStatusEnum)StatusId;
            }
            set
            {
                StatusId = (byte)value;
        ...
        public int StatusId
        {
            ...
            set
            {
                ...
                {
                    TransStatusId = value;
                    this.WorkFlow.AddSequence(value);
        ...
        public APBatchHeader AddToBatch(Batch batch = null)
        {
            ...
            if (existingAPTran == null)
            {
                ...
                LinkedImages.ToList().ForEach(link => {
                    ...
                    batchSeq.Attachment.GetRootFolder().CopyFile(link.File(), createDuplicate);
                });
        ...
        public void LinkImageToTrans(CreditCardImage image)
        {
            ...
            if (link == null)
            {
                ...
                this.PictureStatusId = (int)CMPictureStatusEnum.Attached;
                this.AddLog(CMLogEnum.PictureAdded, string.Format("Transaction picture was added {0}", CMPictureStatusEnum.Attached));                
            }
        ...
        public void FixTransActionErrors()
        {
            ...
            if (this.CodedStatusId == (int)CMTransCodeStatusEnum.AutoCoded && this.TransStatusId == (int)CMTranStatusEnum.Open)
                this.AutoCode();
        }
        ...
        public CreditTransactionCode AddTransactionCode(CCWexImport importLine)
        {
            ...
            if (line == null)
            {
                ...
                line = new CreditTransactionCode()
                {
                    ...
                    Transaction = this,
                    tLineTypeId = (byte)CMCodeLineTypeEnum.Equipment,
                ...
                AddLog(CMLogEnum.AutoCoded, string.Format("Transaction was autocoded"));
                Coding.Add(line);
            }
            return line;
        ...
        public  CreditTransactionLog AddLog(CMLogEnum logType, string description = "")
        {
        ...
        public CreditTransactionCode GetDefaultTransactionCode()
        {
            var transCode = this.Coding.FirstOrDefault();
            if (transCode == null)
            {
                ...
                transCode = new CreditTransactionCode
                {
                    Transaction = this,
        ...
        public void AutoCode()
        {
            ...
            if (CodedStatusId == (int)CMTransCodeStatusEnum.Empty || CodedStatusId == (int)CMTransCodeStatusEnum.AutoCoded)
            {
                var defaultLineTypeId = this.DefaultLineTypeId();
                var defaultGL = this.DefaultGLAcct();
                var defaultPhaseId = this.DefaultJCPhaseId();
                ...
                var coding = GetDefaultTransactionCode();
                ...
                {
                    if (defaultGL != null && 
                        defaultJob == null && defaultPhaseId == null && 
                        defaultEMCostCodeId == null && defaultEMCType == null)
                    {
                        defaultLineTypeId = (byte)CMCodeLineTypeEnum.Expense;
                    }
                    ...
                    if (defaultLineTypeId == null)
                    {
                        ...
                        {
                            coding.LineType = CMCodeLineTypeEnum.Job;
                            ...
                            CodedStatusId = (int)CMTransCodeStatusEnum.AutoCoded;
                            AddLog(CMLogEnum.AutoCoded, string.Format("Transaction was autocoded as Job"));
                        }
                        else if (coding.tGLAcct != null)
                        {
                            coding.LineType = CMCodeLineTypeEnum.Expense;
                            coding.UpdateLineType(coding.LineType);
                            CodedStatusId = (int)CMTransCodeStatusEnum.AutoCoded;
                            AddLog(CMLogEnum.AutoCoded, string.Format("Transaction was autocoded as Expense"));
                        }
                    }
                    else
                    {
                        coding.LineTypeId = defaultLineTypeId;
                        switch ((CMCodeLineTypeEnum)defaultLineTypeId)
                        {
        ...
        public void AddWorkFlowAssignments(bool reset = false)
        {
            ...
            switch (Status)
            {
                case CMTranStatusEnum.Open:
                    WorkFlow.AddEmployee(this.Employee);
                    WorkFlow.AddUsersByPosition("FIN-APMGR");
                    WorkFlow.AddUsersByPosition("FIN-CTRL");

                    break;
                case CMTranStatusEnum.Locked:
                    WorkFlow.CompleteSequence();
                    break;
                case CMTranStatusEnum.APPosted:
                    WorkFlow.CompleteSequence();
                    break;
                case CMTranStatusEnum.RequestedInfomation:
                    WorkFlow.AddEmployee(this.Employee);
                    WorkFlow.AddUsersByPosition("FIN-APMGR");
                    WorkFlow.AddUsersByPosition("FIN-CTRL");
                    break;
    ...
    public static class CreditTransactionExtension
    {
        ...
        {
            if (transaction == null) throw new System.ArgumentNullException(nameof(transaction));
            if (transaction.PictureStatusId == (int)CMPictureStatusEnum.Attached)
            {

            }
            else if (!transaction.IsReceiptRequired())
            {
                if (transaction.PictureStatusId != (int)CMPictureStatusEnum.NotNeeded)
                {
                    transaction.PictureStatusId = (int)CMPictureStatusEnum.NotNeeded;
                    transaction.AddLog(CMLogEnum.PictureAdded, string.Format("Transaction was set to picture not needed {0}", CMPictureStatusEnum.NotNeeded));
                }
            }
            else if (transaction.PictureStatusId == (int)CMPictureStatusEnum.NotNeeded)
            {
                transaction.PictureStatusId = (int)CMPictureStatusEnum.Empty;
                transaction.AddLog(CMLogEnum.PictureAdded, string.Format("Transaction was set to picture needed"));
            }
        }

...
namespace DB.Infrastructure.ViewPointDB.Data
{
    public partial class CreditImport
    {
        ...
        public static CreditImport CreateImport(HttpPostedFileBase fileUpload, VPContext db)
        {
            ...
            var co = db.GetCurrentCompany().HQCo;
        ...
        private void ImportCsvFile(HttpPostedFileBase fileUpload)
        {
            ...
            {
                Source = "Zion";
                var list = csv.GetRecords<ZionCSV>().ToList();
            ...
            {
                Source = "Wex";
                var list = csv.GetRecords<WexCSV>().ToList();
                list.ForEach(e => this.AddLine(e));
                ProcessWexLines();
            }
            ...
            {
                Source = "TXTag";
                var list = csv.GetRecords<TXTagCSV>().ToList();
                list.ForEach(e => this.AddLine(e));
                ProcessTXTagLines();
            }
            ...
            {
                Source = "PRBenefit";
                var list = csv.GetRecords<BenefitsCSV>().ToList();
                list.ForEach(e => this.AddLine(e));
                ProcessPRBenefitLines();
            }
        }

        public ZionImport AddLine(ZionCSV item)
        {
        ...
        public CCWexImport AddLine(WexCSV item)
        {
            var line = new Infrastructure.ViewPointDB.Data.CCWexImport
            {
                CMCo = Co,
                ImportId = ImportId,
        ...
        public ImportTexTag AddLine(TXTagCSV item)
        {

            var line = new Infrastructure.ViewPointDB.Data.ImportTexTag
            {
                CMCo = Co,
                ImportId = ImportId,
        ...
        public PRImportBenefit AddLine(BenefitsCSV item)
        {
            ...
            else if (Source == "Zion")
            {
                ...
                this.ZionLines.ToList().ForEach(e => {
                    if (e.IsError == false)
                    {
                        ...
                        {
                            var transline = trans.AddCreditTransactionLine(e);
                            var transCode = trans.GetDefaultTransactionCode();
                            trans.AutoCode();
                            trans.AutoPictureStatus();
                            //trans.TransAmt = trans.Coding.Sum(sum => sum.GrossAmt ?? 0);
                            if (trans.TransAmt != trans.Coding.Sum(sum => sum.GrossAmt ?? 0))
                            {
                                var i = 0;
                                var subTotal = trans.Coding.Sum(sum => sum.GrossAmt ?? 0);
                            }
                        }
                    }
                    i++;
                ...
                if (newTrans.Any())
                {
                    ...
                    foreach (var item in newTrans)
                    {
                        ...
                        item.Lines.ToList().ForEach(e => e.TransId = transId);
                        transId++;
      //                  db.CreditTransactions.Add(item);
      //                  try
						//{
						//	db.SaveChanges();
